...
class AppModelCreateAPIViewSet(AppViewMixin, CreateModelMixin, AppGenericViewSet):
    """
    Urls Allowed:
        > POST: {endpoint}/
            >> Get data from front-end and creates an object.
...
from apps.common.serializers import AppWriteOnlyModelSerializer, simple_serialize_queryset
    ...
    class _View(AppCreateAPIView):
        ...
        class _Serializer(AppWriteOnlyModelSerializer):
            ...
            def to_representation(self, instance):
                """Return a clean JSON-safe response with file URL."""
                return {
                    "id": str(instance.id),
        ...
        def create(self, request, *args, **kwargs):
            file_size_limit = 3 * 1024 * 1024  # 3 MB in bytes
            ...
            if uploaded_file.size > file_size_limit:
                return self.send_error_response(data={"detail": "File size exceeds the limit of 3 MB"})
...
class AppModelRetrieveAPIViewSet(AppViewMixin, RetrieveModelMixin, AppGenericViewSet):
    """
    Urls Allowed:
        > GET: {endpoint}/<pk>/
            >> Get data from font-end to retrieve an object.
    """

    pass


class AppModelDeleteAPIViewSet(AppViewMixin, DestroyModelMixin, AppGenericViewSet):
    """
    A separate common abstract class for delete model instance
...
class AppModelUpdateAPIViewSet(AppViewMixin, UpdateModelMixin, AppGenericViewSet):
    """
    A separate common abstract class for handling full updates (PUT) on model instances.

    Urls Allowed:
        > PUT: {endpoint}/<pk>/
            >> Fully updates the object identified by the passed `pk`.
        > PATCH: {endpoint}/<pk>/
            >> Partially updates the object identified by the passed `pk`.
    """
